...
@method_decorator(ratelimit(key='ip', rate='7/15m'), name='create')
...
class VerificationView(generics.GenericAPIView):
    ...
    def get(self, request, uid):
        ...
        try:
            #decode JWT and extract username from playload
            secret_key = settings.SECRET_KEY
            playload = jwt.decode(uid, secret_key, algorithms=['HS256'])
            username = playload['username']

            user = get_user_model().objects.filter(username=username).first()
            if user and (timezone.now() - user.date_joined) < timedelta(hours=1):
